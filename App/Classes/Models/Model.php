...
abstract class Model
{
    ...
    {
        if (!$this->isNew($this->id, $this->contact_id) or (static::class == 'App\Models\Contact')) {
            $this->update();
        } else $this->insert();
    }
    ...
    public static function findByContactIdAndId($contact_id, $id){
        ...
        $recordById = $db->query('SELECT ALL * FROM ' . static::TABLE . ' WHERE contact_id = :contact_id AND id = :id', static::class, $data);
        return $recordById;
    }
}